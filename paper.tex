...
\rE_\ell, \ \ell=6, 7                  & \ell &1      & 27, 56         & \text{minimal}       & \rD_{\ell-1}              & 2(\ell-1)      & \ell-1  \end{array}\]
...
First of all notice that $N_{rs}$ is sufficiently large.
...
\begin{enumerate}
 ...
 \item \textit{Case $\Phi = \rB_\ell$, $s=1$, $r=\ell$.} 
  The argument from the previous case shows that $L_{\ell,1}$ contains root subgroups $X_\alpha(I)$, where $\alpha$ varies over the long roots of $\Sigma_1^-$. 
  The inclusion $X_{0, 1}(I) \subseteq L_{\ell, 1}$ for the remaining short root follows from inclusions $X_{-1, 2}(I) \cup X_{2, 1}(I) \subseteq L_{\ell,1}$, $X_{2, 0}(R) \subseteq N_{\ell,1}$
  using Chevalley commutator formula $x_{0, 1}(-ab) = [x_{-1,2}(a),\, x_{2,0}(b)] \cdot x_{2,1}(ab^2).$
 \item \textit{Case $\Phi = \rC_\ell$, $s=1$, $r=\ell$.} 
  Similarly, one can show that $L_{\ell,1}$ contains $X_\alpha(I)$ for all short $\alpha \in \Sigma_1^-$.
  However, the inclusion $X_{-1, 1}(I) \subseteq L_{\ell, 1}$ for the remaining long root does not immediately follow from commutator formulae.
  To verify it we invoke stability conditions once again.
  From~\cref{lemma:DVST} we obtain that $N_{\ell, 1}$ contains $X_{-\alpha\ssub{1}}(R)$ and therefore coincides with the whole $\E(\Phi, R)$, which gives the required assertion.
\end{enumerate}
...
On the other hand, from $\mathcal{Z}(\Sigma_s^-) \subseteq \mathcal{X}^{\EP_s(R)} \subseteq L_{rs}^{N_{rs}} \subseteq L_{rs}$ we conclude that 
 $\E(\Phi, R, I) \subseteq L_{rs}$, which implies the assertion of the theorem.
