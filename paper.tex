...
$\varpi_1$ (in the case of a classical $\Phi_l$) or $\varpi_l$ (in the case $\Phi_l=\rE_l$). 
The former are the natural vector representations of classical groups acting on free modules $V=R^n$ of dimension 
$n=l+1, 2l+1, 2l,2l$ for $\Phi=\rA_l,\rB_l,\rC_l,\rD_l$.
The latter representations have dimension $n=27, 56, 248$ for $l=6,7,8$.

For a representation $\pi$ of $\G(\Phi, R)$ on $V$ we denote by $\Lambda(\pi)$ the set of weights of $\pi$.
It will be convenient to use standard numbering for elements of $\Lambda(\pi)$ in the case when $\Phi$ is classical and $\pi$ is a vector representation (cp.~\cite[§1B]{St78}):
$$\begin{array}{cll}
  1,2,\ldots, l+1 & \text{in the case} & \Phi =\rA_l, \\
  1,2,\ldots l, 0, -l,\ldots, -2, -1 & \text{in the case} & \Phi =\rB_l, \\
  1,2,\ldots l, -l,\ldots, -2, -1 & \text{in the cases}   & \Phi =\rC_l, \rD_l. \\
\end{array}$$
