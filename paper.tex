...
\begin{abstract} 
We formulate and prove relative versions of several classical ``parabolic'' decompositions known in the theory of Chevalley groups over commutative rings.
We apply them to construct new factorizations in terms of $\SL_2$ and $\SL_n$ subgroups and to obtain upper estimates on the width of principal congruence subgroups in terms of Tits---Vaserstein generators.
...
The aim of the present paper is to extend various decomposition of Chevalley groups and their elementary subgroups to their principal congruence subgroups and relative elementary subgroups. We focus on the so-called ``parabolic decompositions'', i.e. decompositions involving parabolic and unipotent factors. The main result is the following

\begin{restatable*}[Dennis---Vaserstein decomposition]{thm}{ThmDennisVaserstein}\label{thm:DennisVaserstein}
Let $\Phi \neq \rE_8$ be a reduced irreducible root system of rank $\ell\geqslant 2$ and let $\{ r, s \}$ be a pair of distinct terminal vertices of the Dynkin diagram of $\Phi$. Denote the distance between $r$ and $s$ by $d$.
...
The term ``Dennis---Vaserstein decomposition'' is due to A.~Suslin and M.~Tulenbaev \cite[Lemma~2.1]{ST76}.
...
We also prove the relative analogues of Gauss and Bass---Kolster decompositions (\cref{thm:srRI1,thm:BassKolster}).

We apply these decompositions to several problems. First, we investigate the factorizations of Chevalley groups in terms of their subsystem subgroups. One of such factorizations if a decomposition into the product of subgroups isomorphic to $\SL(2, R, I)$. We show in \cref{thm:SL2width} that for classical groups over rings satisfying assumptions close to the stable rank $\leqslant2$ there exist such factorizations of length having the leading term $3|\Phi^+|$ in case $\Phi=\rA_\ell, \rC_\ell$ and $4|\Phi^+|$ otherwise.

Another is the decomposition in terms of subgroups of type $\rA_\ell$ of submaximal rank. We show that Dennis---Vaserstein decomposition can be of great help in this problem. Namely, we prove the following
\begin{restatable*}{thm}{ThmSpinSLnProd}\label{thm:spin-sln-prod}
...
Then we obtain the upper bounds on the width of $\E(\Phi, R, I)$ in terms of Vaserstein---Tits generators for various rings (\cref{thm:width}). 

Let us now fix the notations used in the paper.
...
We tend to omit curly braces in the above notations when $J$ is a one or two element set, e.\,g., $\Delta_k=\Delta_{\{k\}}$ and $\Sigma_{i, j}^\pm=\Sigma_{\{i, j\}}^\pm$, etc.
...
Let $\G(\Phi, R)$ be the simply-connected Chevalley group of type $\Phi$ over an arbitrary commutative ring $R$ and let $\E(\Phi, R)$ be its elementary subgroup, generated by the elementary root unipotents $x_\alpha(\xi)$, $\alpha\in\Phi$, $\xi\in R$, see~\cite{VP, St78, S}.
...
The following two equalities will be referred to as the \emph{Levi decomposition}:
...
The proof of the Gauss decomposition presented below is similar to the absolute case \cite[Theorem~5.1]{Sm12}.
...
\Phi=\rB_\ell, \rD_\ell, & \asr(I)\leqslant 2, & N=4\left|\Phi^+\right| - 3\rk(\Phi).
\end{array}\]
Then the principal congruence subgroup $\G(\Phi, R, I)$ can be presented as a product of at most $N$ copies of its (regularly embedded) subgroups isomorphic to $\SL(2, R, I)$.
\end{thm}
...
\end{proof}
The above lemmas together with \cref{cor:bass-kolster-count,thm:Gauss} give the following
\begin{thm}\label{thm:width} Let $I$ be an ideal of a commutative ring $R$.
\begin{thmlist}
\item If $R=\mathcal{O}_S$ is a Dedekind ring of arithmetic type posessing a real embedding and $\Phi$ is classical of rank $\geqslant2$, then 
$W(\G(\Phi, R, I), \mathcal{Z}(\Sigma_\ell^-))$ is finite;
