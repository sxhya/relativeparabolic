...
As a further application of parabolic factorizations, we obtain in~\cref{thm:width} estimates of the width $\E(\Phi, R, I)$  
 with respect to this generating set for some special rings having small stable rank.
...
The fundamental weights $P=\{\varpi_1,\ldots\varpi_\ell\}$ are defined as the basis dual to the simple coroots, i.\,e. $\langle \varpi_i, \alpha_j^{\vee} \rangle = \delta_{ij}$.
...
Let $L$ be the complex simple Lie algebra of type $\Phi$.
To each element $\mu$ of the cone of dominants weights $\mathbb{Z}_{\geq 0}\langle P\rangle$
 one can associate an irreducible finite dimensional complex representation $\pi \colon L \to \mathrm{gl}(V)$ with highest weight $\mu$.
For our purposes it suffices to restrict our attention only to fundamental representations of $L$ (i.\,e. representations with highest weight $\varpi_i$),
 which, moreover, we can assume to be {\it basic} in the sense of~\cite[\S~I.2]{Ma69}. 
 
Denote by $\Lambda(\pi)$ the set of weights of $\pi$. Recall that $V$ admits weight decomposition $V = V_0\oplus \bigoplus_{0\neq\lambda\in \Lambda(\pi)} V_\lambda$,
 moreover, thanks to our assumptions, all $V_\lambda$'s corresponding to nonzero $\lambda$ are $1$-dimensional.
By Chevalley--Ree theorem in any finite-dimensional representation $(\pi, V)$ of $L$ one can choose a basis $v_\lambda \in V_\lambda$,
 with the property that $\pi(e_\alpha^{(m)})(v_\lambda)$ is an integral combination of $v_\lambda$ for every $m\in \mathbb{Z}_{\geq 0}$, $\alpha\in \Phi$.
 
Consequently one can construct a representation $\G_{sc}(\Phi, R)$ on the free $R$-module $V_\mathbb{Z}\otimes R$ called a {\it Weyl module}.
For us it will only be relevant that in the case of a basic representation, there are particularly nice formulae describing the action
 of elementary root unipotents $x_\alpha(\xi)$ on weight spaces $V_\lambda$, $\lambda \in \Lambda(\pi)$, see~\cite[Lemme~2.3]{Ma69},
 which can be, moreover, visualized using the technique of {\it weight diagrams} from \cite{PSV98}.

The following lemma is, in essence, a relative version of Chevalley--Matsumoto decomposition which can be easily deduced from the absolute version (cf. \cite[Theorem~1.3]{St78}).
\begin{lemma}\label{lemma:Chevalley-Matsumoto}
Let $\pi$ be a basic fundamental representation of $\G_{sc}(\Phi, R)$ with highest weight $\varpi_s$.
Assume that $g\in \G_{sc}(\Phi, R, I)$ is such that $(g\cdot v^+)_{\varpi_{s}}=1$, then $g \in \U(\Sigma_s^-, I) \cdot \G_{sc}(\Delta_s, R, I) \cdot \U(\Sigma_s^+, I)$. \end{lemma}
Here by $v^+$ we denote the highest vector of $\pi$ and by $v_\varpi$ the coordinate of $ $
...
Notice that the equality $A_{rs} = \E(\Phi, R, I)$ implies symmetric equality $A_{sr} = \E(\Phi, R, I)$, 
 therefore it suffices to consider only the following possibilities for $\Phi$, $s$, $r$.
...
This is accomplished in the series of lemmas below. For example, the following lemma describes root subgroups 
 contained in $N(A_{rs})$ regardless of whether the stability conditions of~\cref{thm:DennisVaserstein} are satisfied.
...
\end{lemma}
\begin{proof}
Let $\Phi$, $r$ and $s$ be as in Table~\ref{table:dv-reps} and let $\pi \colon \G(\Phi, R) \to \mathrm{GL}(V_R)$ be the Weyl module with highest weight $\varpi_s$.
...
The restriction of $\pi$ to the subgroup $\G(\Delta_r, R)$ decomposes into a sum of Weyl modules for $\G(\Delta_r, R)$,
 moreover, each such summand corresponds to a connected component of the weight diagram of $\pi_R$ in which all bonds marked $r$ are removed. 
Denote by $(\pi', V')$ the summand containing the highest weight vector $v^+$ of $\pi$,
 i.\,e. the summand corresponding to the top-left connected component of the diagram and denote by $\Lambda(\pi')$ the subset of weights belonging to this component.
In all cases under consideration, $\pi'$ is isomorphic to the natural representation of $\G(\Delta_r, R)$.

Denote by $\Gamma$ the subset of weights $\lambda \in \Lambda(\pi')$ incident to the removed bonds (i.\,e. such that $\lambda - \alpha_r \in \Lambda(\pi)$).
...
Now let us show that $A = A_{rs}$.
Let $g$ be an element of $\E(\Delta_r, R, I)$. Applying \cref{lemma:uraction} to the subsystem $\Delta_r$, we find
$x\in\U(\Delta_r^+, I)$ and $y\in \U(\Delta_r^-, I)$ such that $yx\cdot g \in B$, therefore
...
\subseteq \EP_\ell(R, I) \cdot \U(\Phi^-, I) \cdot \EP_1(R, I) \subseteq A_{\ell 1}, \end{multline*} as claimed.
\end{proof}
