...
First of all, notice that Dennis--Vaserstein decomposition follows the stronger Gauss decomposition (see~\cref{thm:srRI1} below).
Thus, we may assume that $\ell>2$ and, in particular, that $\Phi\neq \mathsf{G}_2$.
...
\begin{enumerate} 
 ...
 Since $\Delta_r \cap \Sigma_s^-$ contains roots of the first two types, we 
  only need to verify $X_\delta(I) \subseteq L_{rs}$, which again follows from~\eqref{eq:CCF} with $\beta = \alpha_2 + \alpha_3,\ \gamma = -\alpha_{{\max}}$. 
 \item \textit{Case $\Phi = \rC_\ell$, $s=1$, $r=\ell$.} 
  ...
  Using~\cref{lemma:DVST} below we obtain that $N_{rs}$ contains $X_{-\alpha_1}(R)$ and therefore coincides with $\E(\Phi, R)$.
  The required inclusion now follows from this and~\cref{item-trans1}.
\end{enumerate}
...
To show that $Y=\E(\Phi, R, I)$ we need to verify the following assertions:
\begin{enumerate}
...
Fix a root $\alpha\in\pm\Pi$. Clearly, $\alpha \in \Delta_i$ for $i=1$ or for $i=\ell$, and we can factor $Y$ as follows:
\[ Y=\Hh(\Phi, R, I) \cdot \U(\Delta_i^+, I) \cdot \ldots \cdot \U^\pm(\Delta_i, I) \cdot \U(\Sigma_i^+, I) \cdot \ldots \cdot \U(\Sigma_i^\pm, I). \]
For every  $h \in \Hh(\Phi, R, I)$ for some $s\in I$ one has
