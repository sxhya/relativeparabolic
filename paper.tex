...
In the present paper we study parabolic decompositions of Chevalley groups and their principal congruence subgroups. By a parabolic decomposition we mean a decomposition of a group $G$ into a product of some of its subgroups, where the subgroups in question are either parabolic or related to those in some way (mostly the unipotent radicals or their parts). We are interested in the following three versions of parabolic factorizations:
\begin{itemize}
\item Bass---Kolster decomposition $\G(\Phi,R) = \Par_i\U_i^-\U_i\U_i^-$;
\item Dennis---Vaserstein decomposition $\G(\Phi,R)=\Par_r\U_{rs}^-\Par_s$;
\item Gauss decomposition $\E(\Phi,R)=\B\U^-U$.
\end{itemize}
%TODO: Insert some links to VIP's papers :)

The interest for parabolic factorizations comes from the following two considerations.

The first is related to the fact that over rings of small dimension there is a variety of well-known tools, such as Bruhat decomposition over fields, Gauss decomposition with prescribed semisimple part over local rings, short unitriangular factorizations, etc. For each particular analytic or arithmetic setting there is a specific decomposition theorem (Iwasawa, Cartan and Iwahori decompositions to mention a few). So while groups over small-dimensional rings enjoy plenty of useful factorizations, none of those generalize to rings of large dimension. An this is exactly where parabolic decompositions enter the scene, as they hold for groups over rings satisfying very mild conditions such as the stable rank.

The second is that even for rings of small dimension parabolic factorizations prove useful for applications, see below.

We prove the relative versions of the three decompositions listed above (here the word "relative" refers to the principal congruence subgroups).

%TODO: Insert the statements of our decompositions

The first applications is the problem of bounded generation, where we use Bass---Kolster and Gauss decompositions to reduce the problem to the groups of small rank, where one can do some straightforward calculations. We obtain the following two theorems.
...
Then for any $\Phi$ the with of the congruence subgroup $\G(\Phi, R, I)$ with respect to $z_\alpha$ does not exceed $C_1|\Phi| + C_2$. \end{thm}
%TODO: Write explicit estimate for C_1 and C_2.
The theorem above generalizes extends the results of \cite{VavSmSuUnitrEng}, where such bounds were obtained in the absolute case (i.e. when $I=R$).
\begin{thm}
