...
\end{align*}
It is easy to see that $L(A)$, $R(A)$ and $N(A)$ are subgroups of $G$. The following obvious lemma will be useful in the sequel.
...
Let $\Phi$ be an irreducible root system of rank $\ell$ and $r$ and $s$ be two distinct integers such that $1 \leqslant r, s \leqslant \ell$.
...
Assume, for example, the latter. From the Levi decomposition we obtain $\U(\Sigma_r^- \cap \Sigma_s^-, I)^{X_\alpha(R)} \subseteq \U(\Sigma_s^-, I) \subseteq A_{rs}$, therefore $X_{\alpha}(R) \subseteq N(A_{rs})$.
...
First of all, notice that in the case $\ell=2$, $\sr(I)=1$ Dennis--Vaserstein decomposition can be deduced from Gauss decomposition given by~\cref{thm:srRI1} below.
Thus, without loss of generality we may assume $\ell>2$ and, in particular, that $\Phi\neq \mathsf{G}_2$.

Since the equality $A_{rs} = \E(\Phi, R, I)$ implies $A_{sr} = \E(\Phi, R, I)$, it suffices to consider only the possibilities for $\Phi$, $s$, $r$ listed in~\cref{table:dv-reps} below.
\begin{table}[htb]
...
\rE_\ell, \ \ell=6, 7, 8              & 1    &2      & 27, 133, 3875  & \text{???}           & \rA_{\ell-1}              & ?              & ? \\
...
From \cref{lemma:Stein_reduction} below it follows that $N(A_{rs})$ contains root subgroup $X_{-\alpha_{r}}(R)$, which together with~\cref{lemma:dv-normal} 
 implies that $\EP_s(R) \subseteq N(A_{rs})$, and hence by~\cref{lem:LN} that $\EP_s(R, I) \subseteq L(A_{rs})$.

Our next goal is to verify the inclusion $\U(\Sigma_s^-, I) \subseteq L(A_{rs})$. Our main tools will be~\cref{lem:LN} and \cref{item-trans2}.
...
we immediately obtain that $L(A_{rs})$ contains root subgroups $X_\alpha(I)$ for all $\alpha$ lying in the orbit $O = W(\Delta_s) \cdot (\Sigma^-_s\cap \Delta_r)$.
Thus, we are left to consider four cases in which $O$ does not coincide with $\Sigma^-_s$.
\begin{enumerate} 
 \item \textit{Case $\Phi = \rE_8$, $s=8$, $r=1,2$.}  
 There is only one root in $\Sigma_8^-$ not lying in $O$, namely $-\alpha_{\max}$ with $\shape(\{\alpha_8\}, -\alpha_{\max}) = -2$.
 Since $X_{-\alpha_8}(R) \subseteq N(A_{rs})$, $X_{\alpha_8-\alpha_{\max}}(I) \subseteq L(A_{rs})$ the desired inclusion
  follows from Chevalley commutator formula $x_{\alpha_{\max}}(\pm ab)=[x_{\alpha_8-\alpha_{\max}}(a), x_{\alpha_8}(b)].$
 \item \textit{Case $\Phi = \rE_7, \rE_8$, $s=1$, $r=2$.} ???
 ...
 We need to verify $X_{\beta+\gamma}(I) \subseteq L(A_{rs})$, but this follows from $X_{\gamma}(I) \cup X_{-\alpha_1}(I) \subseteq L_{rs}$,
  $X_{\beta}(R) \subseteq N(A_{rs})$ using Chevalley commutator formula:
 ...
 \begin{itemize}
  ...
  \item $\shape(\{\alpha_4\}, \alpha) = 2$, $\alpha$ is short (there is only one such root: $\delta = -\alpha_1 -2\alpha_2-3\alpha_3-2\alpha_4$).
 \end{itemize}
 Since $\Delta_r \cap \Sigma_s^-$ contains roots of the first two types, we only need to verify $X_\delta(I) \subseteq L(A_{rs})$. 
 But the latter inclusion follows from formula~\eqref{eq:CCF} with $\beta = \alpha_2 + \alpha_3,\ \gamma = -\alpha_{{\max}}$. 
 \item \textit{Case $\Phi = \rC_\ell$, $s=1$, $r=\ell$.} 
  In this case the inclusion $X_{-\alpha_{\max}}(I) \subseteq L(A_{rs})$ does not follow from commutator formulae, therefore we need to invoke stability assumptions one more time.
...
Thus, we have just shown that $\mathcal{X}\subseteq L(A_{rs})$. 
...
Let $\Phi$ be a reduced irreducible root system of rank $\ell > 1$ and let $\Delta_k$, $\Delta_l$ be
the reductive subsystems of $\Phi$ corresponding to a pair of distinct simple roots $\alpha_k$, $\alpha_l \in \Pi$.
Suppose that both relative elementary subgroups $\E(\Delta_i, R, I)$, $i=k, l$ admit Gauss decomposition:
\[ \E(\Delta_i, R, I) = \Hh(\Delta_i, R, I) \cdot \U(\Delta^+_i, I) \cdot \U(\Delta^-_i, I) \cdot \U(\Delta^+_i, I), \quad i=k, l. \]
...
From this and the assumption of the proposition for $\alpha \in \Delta_k \cup \Delta_l$ we obtain that
\begin{multline} \nonumber
 A^{x_\alpha(\xi)} = (\Hh(\Phi, R, I) \cdot A(\Phi^+, I))^{X_\alpha(R)} \subseteq (\Hh(\Phi, R, I) \cdot \E(\Delta_i, R, I) \cdot A(\Sigma_i^+, I))^{x_\alpha(\xi)} \subseteq \\
...
\end{equation}
Thus, $A$ is normalized by root subgroups $X_\alpha(R)$, $\alpha \in \Delta_k \cup \Delta_l$ and hence is normalized by $\E(\Phi, R)$.
On the other hand, from $\U(\Phi^+, I) \cdot A \subseteq A$ and~\cref{lem:LN} we conclude that $\mathcal{X} \cdot Y \subseteq Y$ and, finally,
 that $\E(\Phi, R, I) \cdot Y \subseteq Y$.
\end{proof}
\begin{comment}
\begin{prop}\label{thm:Gauss2}
