...
\item \label{item-trans2} Assume that $\alpha_s\in \Pi$ is a fundamental root of $\Phi$ such that $m_s(\alpha_{\max}) = 1$. 
...
We may assume $\ell>2$ because otherwise $\sr(I)=1$ and \cref{thm:DennisVaserstein} follows the stronger Gauss decomposition (see~\cref{thm:srRI1} below).
...
It is easy to see that $L_{rs}$ contains $\EP_r(R, I)$ and is normalized by $N_{rs}$, i.\,e. ${L_{rs}}^{N_{rs}}\subseteq L_{rs}$. Indeed, for $g\in N_{rs}$, $h\in L_{rs}$ one has
...
\begin{enumerate} 
 ...
 There is only one root in $\Sigma_8^-$ not lying in $O$, namely $-\alpha_{\max}$ with $\shape(8,-\alpha_{\max}) = -2$.
 Since $X_{-\alpha_8}(R) \subseteq N_{rs}$, $X_{\alpha_8-\alpha_{\max}}(I) \subseteq L_{rs}$ the desired inclusion
  follows from Chevalley commutator formula $x_{\alpha_{\max}}(\pm ab)=[x_{\alpha_8-\alpha_{\max}}(a), x_{\alpha_8}(b)].$
 
 \item \textit{Case $\Phi = \rB_\ell$, $s=1$, $r=\ell$.} 
 Only the sole short root of $\Sigma_1^-$ is not contained in $O$. Consider the following two roots of $\Phi$:
 ...
 Since $\Delta_r \cap \Sigma_s^-$ contains roots of the first two types, we 
  only need to verify $X_\delta(I) \subseteq L_{rs}$, but the latter again follows from~\eqref{eq:CCF} with $\beta = \alpha_2 + \alpha_3,\ \gamma = -\alpha_{{\max}}$. 
 \item \textit{Case $\Phi = \rC_\ell$, $s=1$, $r=\ell$.} 
  ...
  In this case the inclusion $X_{-\alpha_{\max}}(I) \subseteq L_{rs}$ does not immediately follow from commutator formulae
...
The subset $\Lambda(\pi') \subseteq \Lambda(\pi)$ corresponds to the component of the diagram that contains the vertex corresponding to the highest weight.
