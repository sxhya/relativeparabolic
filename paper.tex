...
This paper studies decompositions of Chevalley groups over commutative rings (see e.\,g.~\cite{VP} and references therein for the introduction to the theory of Chevalley groups over rings).
Recall that classical decompositions of Chevalley groups over fields such as e.\,g. Bruhat decomposition do not generalize to groups over more general classes of rings.
...
Main interest in these decompositions comes mainly from the study of stability problems for $\K_1$-function modeled on Chevalley groups (see e.\,g.~\cite{St78, Si13}).

The first goal of this paper is to prove analogues of these decompositions for congruence subgroups of Chevalley groups of level $I \trianglelefteq R$
 while making ring-theoretic assumptions only about the ideal $I$ and not the ring $R$ itself.
...
\end{thm}
The notation for the groups and ring-theoretic invariants appearing in the above statement is introduced in Sections~\ref{sec:preliminaries} and~\ref{sec:stability-conditions}.

Our further results in this direction are relative analogues of Gauss and Bass--Kolster decompositions (see~\cref{thm:srRI1,thm:BassKolster}).
%TODO: For what?
Another goal of this paper is to apply parabolic decompositions for obtaining the estimates of the width of relative Chevalley groups in terms of certain generating sets.
For example, in~\cref{thm:width} we obtain upper estimates on the width of $\E(\Phi, R, I)$ with respect to Tits--Vaserstein generators for several small-dimensional rings. 
Also, as a corollary of Bass--Kolster decomposition we obtain upper estimates of the number of factors in the presentation of a classical Chevalley group
 as a product of its subsystem subgroups $\SL(2, R, I)$, see~\cref{thm:SL2width}.
Moreover, as a corollary of~\cref{thm:DennisVaserstein} we obtain the following theorem, which is inspired by the main result of~\cite{Nik07}.
...
\begin{enumerate} 
 ...
 There are three possibilities for the $s$-shape and length of $\alpha\in \Sigma_4^-$: 
 \begin{itemize}
  ...
  \item $\shape(\{\alpha_4\}, \alpha) = 2$, $\alpha$ is short (there is only one such root, denote it by $\gamma$).
 \end{itemize}
