...
\begin{thmlist}
 ...
 \item $\sr(I) \leqslant d$ otherwise.
\end{thmlist}
...
We call $\shape(\{s\}, \beta)$ simply the $s$-shape of $\beta$.
...
We may assume $\ell>2$ because otherwise $\sr(I)=1$ and \cref{thm:DennisVaserstein} is a corollary of the stronger Gauss decomposition (see~\cref{thm:srRI1} below).
...
Denote by $N_{rs}$ the subgroup of $\E(\Phi, R)$, consisting of elements normalizing the above subset:
\[ N_{rs} = \left\{ g\in \E(\Phi, R)\ \middle|\ g \cdot A_{rs} \cdot g^{-1} \cup g^{-1} \cdot A_{rs} \cdot g \subseteq A_{rs}\right\}. \]

\begin{lemma}\label{lemma:dv-normal} 
For every $\alpha \in \Delta_{r, s} \cup \Phi^+ $ one has $X_\alpha(R) \subseteq N_{rs}$. \end{lemma}
\begin{proof}
Notice that for every $i$ the group $\EP_i(R, I)$ is normalized by $\EP_i(R)$, hence it is normalized by $X_\alpha(R)$, $\alpha \in S_i^+$.
Since $\E(\Delta_{r, s}, R)$ normalizes $\U(\Sigma_r^- \cap \Sigma_s^-, I)$, we obtain the assertion for $\alpha \in \Delta_{r, s}$.

Now let $\alpha$ be a positive simple root. It is clear that $\alpha$ lies either in $\Delta_r$ or $\Delta_s$.
Assume, for example, the latter. From the Levi decomposition we get $\U(\Sigma_r^- \cap \Sigma_s^-, I)^{X_\alpha(R)} \subseteq \U(\Sigma_s^-, I) \subseteq A_{rs}$,
consequently $X_{\alpha}(R) \subseteq N_{rs}$.
Now it is clear that $N_{rs}$ contains the subgroup $\U(\Phi^+, R)$ generated by $X_{\alpha}(R)$, $\alpha\in\Pi$, which implies the assertion.
%TODO: Why the positive unipotent radical is generated by simple root subgroups???
\end{proof}

\begin{proof} [Proof of \cref{thm:DennisVaserstein}]
Since the equality $A_{rs} = \E(\Phi, R, I)$ implies $A_{sr} = \E(\Phi, R, I)$, it suffices to consider only the following possibilities for $\Phi$, $s$, $r$: % listed in~\cref{table:dv-reps}.
\begin{table}[htb]
...
\rE_\ell, \ \ell=6, 7, 8              & \ell &2      & 27, 56, 248    & \text{minimal}       & \rA_{\ell-1}              & \ell           & 3       \\ 
\rE_\ell, \ \ell=6, 7, 8              & \ell &1      & 27, 56, 248    & \text{minimal}       & \rD_{\ell-1}              & 2(\ell-1)      & \ell-1 \\
...
Denote by $L_{rs}$ the monoid consisting of elements $g\in \E(\Phi, R, I)$ such that $g \cdot A_{rs} \subseteq A_{rs}$.
...
\begin{equation}\label{rel:NnormL} h^g \cdot A_{rs} = g^{-1} \cdot h \cdot g \cdot A_{rs} \cdot g^{-1} \cdot g \subseteq g^{-1} \cdot h \cdot A_{rs} \cdot g \subseteq {A_{rs}}^g \subseteq A_{rs}.\end{equation}

\cref{lemma:Stein_reduction} below guarantees that $N_{rs}$ contains the root subgroup $X_{-\alpha_{r}}(R)$, which together with~\cref{lemma:dv-normal} 
 implies that $\EP_s(R) \subseteq N_{rs}$ and consequently that $\EP_s(R, I) \subseteq L_{rs}$. Indeed for $g\in \EP_s(R, I) \subseteq \EP_s(R)$ one has
$g \cdot L_{rs} = g \cdot L_{rs} \cdot g^{-1} \cdot g \subseteq L_{rs} \cdot g \subseteq L_{rs}.$

Our next goal is to verify the inclusion $\U(\Sigma_s^-, I) \subseteq L_{rs}$. Our main tools will be formula~\eqref{rel:NnormL} and \cref{item-trans2}.
Since $\widetilde{W}(\Delta_s) \subseteq \E(\Delta_s, R) \subseteq N_{rs}$ and 
 $X_{\alpha}(I) \subseteq L_{rs}$ for $\alpha \in \Sigma^-_s\cap \Delta_r$, 
we immediately obtain that $L_{rs}$ contains root subgroups $X_\alpha(I)$ corresponding to $\alpha$
 lying in the orbit $O = W(\Delta_s) \cdot (\Sigma^-_s\cap \Delta_r)$.
Thus, we are left to consider four cases in which $O$ does not coincide with $\Sigma^-_s$. 
%All the possibilities for $\Phi$, $s$ and $r$ for which $O \neq \Sigma_s^-$ fall into one of the following cases:
\begin{enumerate}
 %\item \textit{Case $m_s(\alpha_{\mathrm{max}})=1$ and $\Phi$ is simply laced.}  In this case $O = \Sigma_s^-$ and there is nothing else to prove.
 \item \textit{Case $\Phi = \rE_8$, $s=8$, $r=1,2$.} 
 %The argument from the previous case shows that $L_{rs}$ contains root subgroups $X_\alpha(I)$, where $\alpha$ varies over the roots of $\Sigma_8^-$ having $8$-shape $1$.
 In this case there is only one root in $\Sigma_8^-$ not lying in $O$, namely $-\alpha_\mathrm{max}$ with $\shape(8,-\alpha_\mathrm{max}) = -2$.
 Since $X_{-\alpha_8}(R) \subseteq N_{rs}$, $X_{\alpha_8-\alpha_\mathrm{max}}(I) \subseteq L_{rs}$ the desired inclusion
  follows from Chevalley commutator formula $x_{\alpha_\mathrm{max}}(\pm ab)=[x_{\alpha_8-\alpha_\mathrm{max}}(a), x_{\alpha_8}(b)].$
 
 \item \textit{Case $\Phi = \rB_\ell$, $s=1$, $r=\ell$.} 
 In this case only the sole short root of $\Sigma_1^-$ is not contained in $O$. Consider the following two roots of $\Phi$:
 \begin{equation}\nonumber \beta = \alpha_2 + \ldots + \alpha_\ell,\ \gamma = -\alpha_1 - 2\alpha_2 - \ldots -2\alpha_\ell.\end{equation}
 We need to verify $X_{\beta+\gamma}(I) \subseteq L_{rs}$, but this follows from $X_{\gamma}(I) \cup X_{-\alpha_1}(I) \subseteq L_{rs}$, $X_{\beta}(R) \subseteq N_{rs}$ using Chevalley commutator formula:
  \begin{equation} \label{eq:CCF} x_{\beta+\gamma}(-ab) = [x_{\gamma}(a),\, x_{\beta}(b)] \cdot x_{\gamma + 2\beta}(ab^2). \end{equation}
 \item \textit{Case $\Phi = \rF_4$, $s=4$, $r=1$.} In this case there are three possibilities for the $s$-shape and length of a root from $\Sigma_4^-$: 
 \begin{itemize}
  \item $\shape(4, \alpha) = 1$, $\alpha$ is short (there are 8 such roots);
  \item $\shape(4, \alpha) = 2$, $\alpha$ is long (there are 6 such roots);
  \item $\shape(4, \alpha) = 2$, $\alpha$ is short (there is only one such root: $\delta = -\alpha_1 -2\alpha_2-3\alpha_3-2\alpha_4$).
 \end{itemize}
 Since $\Delta_r \cap \Sigma_s^-$ contains roots of the first two types, we 
  only need to verify $X_\delta(I) \subseteq L_{rs}$, but the latter again follows from~\eqref{eq:CCF} with $\beta = \alpha_2 + \alpha_3,\ \gamma = -\alpha_{\mathrm{max}}$. 
 \item \textit{Case $\Phi = \rC_\ell$, $s=1$, $r=\ell$.} 
  %Similarly, one can show that $L_{rs}$ contains $X_\alpha(I)$ for all short $\alpha \in \Sigma_1^-$.
  In this case the inclusion $X_{-\alpha_\mathrm{max}}(I) \subseteq L_{rs}$ does not immediately follow from commutator formulae
   and we need to invoke stability conditions once again.
  From~\cref{lemma:DVST} we obtain that $N_{rs}$ contains $X_{-\alpha_1}(R)$ and therefore coincides with $\E(\Phi, R)$, which implies the assertion.
\end{enumerate}

Thus, we have just shown that $\mathcal{X}\subseteq L_{rs}$. 
Recall from~\cite[Theorem~3.4]{S} that the group $\E(\Phi, R, I)$ is generated by $\mathcal{Z}(\Sigma_s^-)$.
On the other hand, from $\mathcal{Z}(\Sigma_s^-) \subseteq \mathcal{X}^{\EP_s(R)} \subseteq L_{rs}^{N_{rs}} \subseteq L_{rs}$ we conclude that 
 $\E(\Phi, R, I) \subseteq L_{rs}$, which completes the proof.
%Thus, to prove our assertion it suffices to show that $L_{rs}$ contains $\mathcal{Z}(\Sigma_s^-)$.
%From the inclusion  and \eqref{rel:NnormL} it follows that we only need to verify the inclusion $\mathcal{X} \subseteq L_{rs}$.
%In turn, the last inclusion follows from the same transitivity argument as above if we use the fact that $X_\alpha(I) \subseteq \EP_r(R, I) \subseteq L_{rs}$ for $\alpha \in \Delta_r \cup \Sigma_r$.
%For $\Phi=\rB_\ell$ we need to apply Chevalley commutator formula once again:
...
In all the cases under consideration, $\pi'$ is isomorphic to the natural representation of $\G(\Delta_r, R)$.
...
\end{lemma}
\begin{proof}
Denoteb by $C$ the set consisting of elements $g \in \EP_s(R, I)$ for which matrix entries $(g_{i, 2})$, $i=2, \ldots, \ell$ form an $I$-unimodular column of height $\ell-1$
