...
Let $\Phi$ be a either a classical irreducible root system of rank $\ell\geqslant 2$ or $\Phi=\rE_7, \rE_8$. 
Let $\{ r, s \}$ be a pair of distinct endnodes of the Dynkin diagram of $\Phi$.
...
\begin{thmlist}
 ...
 \item $\sr(I) \leqslant d$ for $\Phi = \rF_4$ with $\{r,s\} = \{1, 4\}$.
\end{thmlist}
...
Denote by $N_{rs}$ the subgroup of $\E(\Phi, R)$, consisting of elements normalizing the subset $A_{rs}$, i.\,e.
...
%which together with \cref{lemma:dv-normal} implies that $N_{rs}$ contains the Levi subgroup $\E(\Delta_s, R)$.
% 
 %$\EP_s(R) \subseteq N_{rs}$

The next step of the proof is to show that $\U(\Sigma_s^-, I) \subseteq L_{rs}$.
...
\begin{enumerate}
 \item \textit{Case $m_s(\alpha_{\mathrm{max}})=1$ and $\Phi$ is simply laced.}
  In this case all the roots of $\Sigma^-_s$ have the same $s$-shape and length.
  Since $\widetilde{W}(\Delta_s) \subseteq \E(\Delta_s, R) \subseteq N_{rs}$ and $X_{-\alpha_s}(I) \subseteq \EP_r(R, I) \subseteq L_{rs}$,
   the required inclusion $\U(\Sigma_s^-, I) \subseteq L_{rs}$ follows from \cref{item-trans2}.  
 \item \textit{Case $\Phi = \rB_\ell$, $s=1$, $r=\ell$.} 
  The argument from the previous case shows that $L_{rs}$ contains root subgroups $X_\alpha(I)$, where $\alpha$ varies over the long roots of $\Sigma_1^-$.
  Consider the following two roots of $\Phi$:
  \begin{equation}\nonumber \beta = \alpha_2 + \ldots + \alpha_\ell,\ \gamma = -\alpha_1 - 2\alpha_2 - \ldots -2\alpha_\ell.\end{equation}
  We need to verify the inclusion $X_{\beta+\gamma}(I) \subseteq L_{rs}$ for the remaining short root of $\Sigma_1^-$.
  But the latter follows from $X_{\gamma}(I) \cup X_{-\alpha_1}(I) \subseteq L_{rs}$, $X_{\beta}(R) \subseteq N_{rs}$ using Chevalley commutator formula:
 ...
 \begin{itemize}
  \item $\shape(s, \alpha) = 1$, $\alpha$ is short (there are 8 such roots);
  \item $\shape(s, \alpha) = 2$, $\alpha$ is long (there are 6 such roots);
  \item $\shape(s, \alpha) = 2$, $\alpha$ is short (there is only one such root: $\delta = -\alpha_1 -2\alpha_2-3\alpha_3-2\alpha_4$).
 \end{itemize}
 Since $\Delta_r \cap \Sigma_s^-$ contains roots of the first two types, we have the neccessary inclusions for the root subgroups of corresponding type via the same argument as above.
 To verify the inclusion for the remaining short root subgroup invoke~\eqref{eq:CCF} with $\beta = \alpha_2 + \alpha_3,\ \gamma = -\alpha_{\mathrm{max}}$. 
 \item \textit{Case $\Phi = \rC_\ell$, $s=1$, $r=\ell$.} 
  ...
  However, the inclusion $X_{-1, 1}(I) \subseteq L_{rs}$ for the remaining long root does not immediately follow from commutator formulae and to verify it we need to invoke stability conditions once again.
  From~\cref{lemma:DVST} we obtain that $N_{rs}$ contains $X_{-\alpha_1}(R)$ and therefore coincides with $\E(\Phi, R)$, which gives the required assertion.
\end{enumerate}
