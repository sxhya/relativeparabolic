...
In section~\ref{sec:boundgen} we deduce from an earlier result of O.~Tavgen that the width $\E(\Phi, R, I)$ with respect to this generating set is
 finite for Dedeking rings of arithmetic type having a real embedding.
...
\begin{align*} 
 L(A) & =  \left\{ g\in G\ \middle |\ g \cdot A = A \right\}\!,\\
 R(A) & =  \left\{ g\in G\ \middle |\ A \cdot g = A \right\}\!,\\
...
Denote by $(\pi', V')$ the summand containing the highest weight vector $v^+$ of $\pi$,
 i.\,e. the summand corresponding to the top-left connected component of the diagram. Denote by $\Lambda(\pi')$ the subset of weights belonging to this component.
In all cases under consideration, $\pi'$ is isomorphic to the natural representation of $\G(\Delta_r, R)$.

Denote by $\Gamma$ the subset of weights $\lambda \in \Lambda(\pi')$ incident to the removed bonds (i.\,e. $\lambda$ such that $\lambda - \alpha_r \in \Lambda(\pi)$).
