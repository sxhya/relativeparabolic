...
Let $\Phi \neq \rE_8$ be a reduced irreducible root system of rank $\ell\geqslant 2$ and let $\{ r, s \}$ be a pair of distinct terminal vertices of the Dynkin diagram of $\Phi$.
...
% For classical groups over rings satisfying suitable dimension assumptions we show the existence of 
% factorizations with $3|\Phi^+|$ in case $\Phi=\rA_\ell, \rC_\ell$ and $4|\Phi^+|$ otherwise, see
...
Finally, we obtain upper bounds on the width of $\E(\Phi, R, I)$ with respect to Vaserstein---Tits generators for several important classes of small-dimensional rings (see~\cref{thm:width}). 
...
By a consideration of the weight diagrams we determine the number of elements in $\Lambda(\pi')$ and $\Gamma$ (see~Table~\ref{table:dv-reps}).
...
We now turn our attention to the proof of \cref{thm:spin-sln-prod} and the study of factorizations in terms of subsystem subgroups of large rank.
...
In this section we apply parabolic factorizations to the problem of bounded generation.
The main result of this section gives an upper bound on the width of $\E(\Phi, R)$ in terms of Tits generators and is, thus,
 a quantitative analogue of \cite[Theorem~3.4]{S} and a relative analogue of the results of~\cite{VseUnitrZ1p, VavSmSuUnitrEng}.
...
\end{thmlist}
\end{thm}
Here $W(G, X)$ denotes \emph{the width of $G$ with respect to a generating set $X$}, i.\,e. the smallest natural number $N$ such that every element of $G$ is a product of at most $N$ elements of $X$ or their inverses.
