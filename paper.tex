...
Before we proceed with the proof of~\cref{thm:DennisVaserstein} let us remark that in the special case $\ell=2$
 $\E(\Phi, R, I)$ admits Gauss decomposition (see~\cref{thm:srRI1} below).
...
\end{lemma}
\begin{proof}
Let $\pi$ be the fundamental representation of $\G(\Phi, R)$ corresponding to the highest weight $\varpi_s$ (see Table~\ref{table:dv-reps}).
Denote by $\Lambda(\pi)$ its set of weights.
...
We refer the reader to~\cite{PSV98}, where this technique is introduced properly and where the weight diagrams have been drawn for 
 almost all the representations appearing in~\cref{table:dv-reps}.
...
From Lemmas~\ref{lemma:Stein_reduction} and~\ref{lemma:dv-normal} it follows that $N(A_{rs})$ contains $\EP_s(R)$, 
 hence by~\cref{lem:LN} we obtain that $\EP_s(R, I) \subseteq L(A_{rs})$.
...
Thus, we are left to consider five cases when $O$ does not coincide with $\Sigma^-_s$.
\begin{enumerate} 
 ...
 \item \textit{Case $\Phi = \rE_7,$ (resp. $\Phi= \rE_8$), $s=1$, $r=2$.}
 There is only one (resp. there are $14$) roots in $\Sigma_1^-$ with $1$-shape $-2$, which are not contained in $O$ for this reason.
 Denote by $\beta$ the maximal root of $\Phi$ (resp. the maximal root of $\Delta_8 \cong \rE_7$).
 It is easy to check that $\shape(\{\alpha_1\}, \beta) = 2$ but $-\beta+\alpha_1 \in O$, therefore the assertion can be obtained via the same argument. 
...
From this and the assumption of the proposition we obtain for $\alpha \in \Delta_k \cup \Delta_l$ that
\begin{multline} \nonumber
...
\end{equation}
Thus, $A$ is normalized by root subgroups $X_\alpha(R)$, $\alpha \in \Delta_k \cup \Delta_l$ and therefore is normalized by $\E(\Phi, R)$.
