...
\begin{cor} \label{cor:embeddingBD}
 Denote by $i$ the standard embedding $\G(\rB_{\ell-1}, R) \hookrightarrow \G(\rD_\ell, R)$. 
...
\begin{proof}
 ...
 Indeed, denote by $\sigma$ the automorphism of $\E(\rD_\ell, R)$ induced by the symmetry of its Dynkin diagram.
 Write $u$ as a product $u_1 \cdot u_2$, where $u_1 \in \U(\Sigma_\ell^+\cap\Delta_{\ell-1}, I)$ and $u_2 \in \U(\Sigma_\ell^+ \cap \Sigma_{\ell-1}^+, I)$ and set $u' = \sigma(u_1)$.

 Now by \cref{item:asrUnipD} one finds a suitable $u \in \U(\Sigma_\ell^+, I) \leqslant \E(\rD_\ell, R, I)$ such that $(u\cdot v)^+$ is unimodular.
 Since $u'$ lies in $\G(\Delta_\ell, R, I)$ the column $(uu' \cdot v)^+$ is also unimodular, as claimed.
\end{proof}
...
Since $\U(\Sigma_r^- \cap \Sigma_s^-, I)$ is normalized by $\E(\Delta_{\{r, s\}}, R)$, we obtain the result for $\alpha \in \Delta_{r, s}$.

Now if $\alpha$ lies in $\Phi^+ \setminus (\Sigma_r^+ \cap \Sigma_s^+)$, it is contained either in $\Delta_r$ or $\Delta_s$.
...
Denote by $L_{rs}$ the set consisting of elements $g\in \E(\Phi, R, I)$ such that $g \cdot A_{rs} \subseteq A_{rs}$.
...
Our next goal is to demonstrate the inclusion $\U(\Sigma_s^+, R) \subseteq N_{rs}$ (which would imply $\EP_s(R) \subseteq N_{rs}$).
...
\begin{enumerate}
 ...
 \item \textit{Case $\Phi = \rB_\ell$.} Using the argument from the previous case, we get that $N_{rs}$ contains the subgroup generated by the root subgroups $X_\alpha(R)$, where $\alpha$ varies over the long roots of $\Sigma_s^+$. 
  It remains to prove the inclusion $X_{1, 0}(R) \subset N_{rs}$.
  From Chevalley commutator formula we obtain
\begin{equation*} \label{rel:CCF-specBC}
x_{1, 0}(ab) = [x_{1, 2}(a), x_{2, 0}(b)] \cdot x_{1, -2}(-a b^2).
\end{equation*}
  Since $X_{2, 0}(R)$, $X_{1, -2}(R)$ and $X_{1,2}(R)$ are contained in $N_{rs}$, the inclusion $X_{1, 0}(R) \subseteq N_{rs}$ follows.
 ...
 In fact, we prove a stronger assertion, namely that $N_{rs}$ contains $X_{-\alpha\ssub{s}}(R)$ and, consequently, that $N_{rs} = \E(\Phi, R)$.
\end{enumerate}
...
From the inclusion $\mathcal{Z}(\Sigma_s^-) \subseteq \mathcal{X}^{\EP_s(R)}$ and \eqref{rel:NnormL} it follows that we only need to verify that $\mathcal{X} \subseteq L_{rs}$.
...
Applying \cref{item:asrUnipC} or \cref{cor:embeddingBD} (in the cases $\Phi=\rC_\ell$ or $\Phi=\rF_4$, respectively), %to the subgroup $\G(\Delta_s, R, I)$,
