...
%Notice that if we could choose $b$ to be persymmetric (i.\,e. such that $(pb)^t=pb$) the assertion of the lemma would follow. Indeed, in such a case $g$ would lie in $\U(\Sigma_\ell, I)$.
%Choose a diagonal matrix $d' \in \M(\ell, R)$ such that $du^- = -(bd)^tu^-$ and set $a = bd + (bd)^t - d'.$
%Then obviously, $u^++pa\cdot u^-=u^++p(bd)\cdot u^-$, $a=a^t\in \M(n,I)$.
%Остаётся воспользоваться замечанием~\ref{UnitarySpecialCases} и леммой~\ref{unipAction}.
