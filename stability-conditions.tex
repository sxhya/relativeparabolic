...
The last estimate further improves to $\sr(F[x_1, \ldots, x_n])\leqslant n$ for $F$ algebraic over a finite field (see~\cite[Corollary~17.4]{VasSusSerreProblem}).
...
For example, $\sr(F[x,y])=3$ for every $F$ with $\K_2^M(F)\neq0$ (see~\cite{KrusenmeyerThesis}) and $\sr(F[x_1, \ldots, x_n])=n+1$ for $F\subseteq\mathbb{R}$, see~\cite[Theorem~8]{Va71}.
