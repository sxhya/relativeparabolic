In this section we define stability conditions used in the statements of decomposition results from section~\ref{sec:factorizations}.
First, we recall the notion of stable rank of a ring without unit introduced by L.~Vaserstein in~\cite{Va69, Va71}.
As we are mainly concerned with Chevalley groups, our exposition of stable ranks is neccessarily limited to the case of a commutative ring.
Our next goal is to define the relative version of the absolute stable rank condition which generalizes the definition of~\cite{EO, MKV}.
In the end of this section we state several technical lemmas describing the action of certain unipotent radicals on unimodular columns under stability conditions.
