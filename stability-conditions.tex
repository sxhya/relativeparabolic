...
Another corollary of \cref{prop:asr-dimmax} is an estimate for the stable rank of the polynomial rings over a field.
...
Notice that the column $$v'=(v_1,\ldots,v_\ell, v_{-\ell}^2,\ldots,v_{-1}^2)^t$$ is also $I$-unimodular.
By the definition of the relative stable rank we can find a matrix $b\in \M(\ell,I)$
such that the upper half $v''_+$ of the vector $v''= g(b) \cdot v'$, where 
$g(b)=\left(\begin{smallmatrix} e_\ell & p \cdot b \\ 0 & e_{\ell} \end{smallmatrix}\right)$, is $I$-unimodular.
It is clear that $v''_+ = v'_+ + pb v'_- = v_+ + pbd v_-$. 
Consider the matrix $a \in M(\ell, I)$ defined by 
\begin{align*}
 & a_{ij} = b_{ij} v_{-j} + b_{ji} v_{-i},\ j\neq i,\\
 & a_{ii} = b_{ii} v_{-i} - \sum\limits_{\mathclap{\substack{j=1 \\ j\neq i}}}^\ell b_{ji} v_{-j}.
\end{align*}
It is clear that the matrix $pa$ is persymmetric and that $g(a) \in \U(\Sigma_\ell, I)$. 
On the other hand, direct calculation shows that $(g(a)\cdot v)_+=v_+ + pav_- = v_+ + pbdv_- = v''_+ \in \Ums(\ell, I)$.
%$$(av_-)_k = \sum\limits_{i=1,\ i\neq k}^\ell \left( b_{ki} v_{-i} + b_{ik} v_{-k} \right)v_{-i}  +
%\left(b_{kk}v_{-k} - \sum\limits_{i=1,\ i\neq k}^\ell b_{ik} v_{-i}\right)v_{-k} = \sum\limits_{i=1}^\ell b_{ki} v_{-i}^2 = (bdv_-)_k$$
