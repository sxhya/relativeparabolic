...
$$N_{rs} = \{ g\in \E(\Phi, R) \mid g \cdot A_{rs} \cdot g^{-1} = A_{rs} \};\quad L_{rs}= \{ g\in \E(\Phi, R, I) \mid g \cdot A_{rs} = A_{rs} \}.$$
...
In the case $\Phi=\rE_\ell$, $r=1$ the subsystem $\Delta_r$ has type $\rD_{\ell-1}$ and $|\Gamma_0|=\ell-1$.
...
To prove the second condition notice that by the definition of $\Gamma_0$ for any $s\in I$, $ g\in\widetilde{L_r}$ one has $x_{-\alpha_r}(s) \cdot g \cdot v^+ = g \cdot v^+$ hence
...
Since $\EP_s \subseteq N_{rs}$ inclusions for the first family of elements follow from those for the second one.
...
By our assumptions on $\Phi$ we have $m_s(\alpha\ssub{\mathrm{max}})=1$ hence every two roots $\alpha, \beta \in \Sigma^-_s$ 
have the same $s$-shape and
%(i.\,e. $\shape(\{s\}, \alpha = \shape(\{s\}, \beta)$).
hence by Lemma~\ref{lemma:abs} are interchanged under the action of $W(\Delta_s)$ if their length is equal. % $\alpha$ and $\beta$ if their length is equal.
...
In the case $\Phi=\rB_\ell$ we have to show additionally that $X_{-\alpha^<_{\mathrm{max}}}(I) \leq L_{rs}$, where $\alpha^<_{\mathrm{max}}$
denotes the maximal short root of $\rB_\ell$.
By Chevalley commutator formula we have:
$$t_{-{\alpha^<_{\mathrm{max}}}}(s) = t_{-1,0}(s) = t_{1,-2}(s) \cdot {}^{t_{-2, 0}(1)}\!t_{-1,-2}(-s) \cdot t_{-1, 2}(\xi),\ s\in I.$$
On the other hand, $t_{-2,0}(1) \in \EP_s$ while we have already shown $X_{-1,-2}(I), X_{-1, 2}(I) \subseteq L_{rs}$.
