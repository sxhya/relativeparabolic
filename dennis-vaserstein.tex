...
First of all, notice that in the case $\Phi=\rG_2$ \cref{thm:DennisVaserstein} formally follows from \cref{thm:srRI1} and identities~\ref{rel:h-comm}--\ref{rel:h-w}.
Since $A_{rs} = \E(\Phi, R, I)$ implies $A_{sr} = \E(\Phi, R, I)$, 
it suffices to consider only the following choices for $\Phi$, $r$ and $s$.
\begin{table}[htb]
