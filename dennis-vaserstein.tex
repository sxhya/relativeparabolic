...
\begin{enumerate}
 ...
 \item \textit{Case $\Phi = \rB_\ell$.} Using the argument as in the first case we get $\U(\Sigma_s^{+>}, R) \subseteq N_{rs}$.
  ...
  We already know that $X_{2, 0}(R)$ and $X_{1,-2}(R)$ are contained in $N_{rs}$ hence we obtain $X_{1,0}(R) \subseteq N_{rs}$, as claimed.
...
The subset $\Lambda(\pi') \subseteq \Lambda(\pi)$ corresponds to the component of the diagram containg the vertex marked $\varpi_s$.
